...
**FCA** 是一个面向多租户、基于证据 grounding 的 AI 工程项目，探索如何把周期性 FP&A performance commentary 转化为一个可 review 的 AI analyst workflow。Commentary 是 visible output；真正的工程重点是它背后的 recurring performance explanation workflow：判断什么变了、什么值得 call out、证据在哪里、哪些数字需要 calculation、哪些内容需要 human review。
...
一个关键洞察是：finance commentary pattern 本质上是 finance team 的 operating standard：callout rules、materiality thresholds、driver-selection logic、metric definitions、formulas、preferred phrasing、reviewer expectations 和 approval patterns。

## 2. 为什么这个问题重要

很多 AI writing tools 都可以生成一段看起来像 financial commentary 的文字。但在真实 finance workflow 里，更难的问题是：这个 AI 系统能不能判断什么值得写、把每个关键财务 claim 绑定到证据、标记 unsupported 或 calculation-boundary 的内容、区分表格数据可支持的 claim 和需要外部业务背景的 claim，并产出能支持人工 review 的审计材料。

FCA 是围绕这个 system-level challenge 设计的。
...
| 团队需要标准化 reporting language | Consistent structure 和 terminology 能帮助 reviewer、manager 和 reader 更快理解 recurring metrics | 学习并保留超越 writing style 的 tenant-specific reporting standard |
...
普通 RAG 系统通常是 retrieve historical examples，然后让 LLM 写一段新的 paragraph。FCA 对 retrieval 的使用方式不同：historical examples 用来支持 tenant pattern learning，而 current-period evidence 用来 ground 和 audit 具体 commentary claims。
...
已有 FP&A copilots、BI tools 或 EPM narrative features 可能已经能帮助用户更快 summarize / read data。FCA 关注的是 variance 被 summarize 之后还必须完成的工作：执行 commentary standards、把 statements link 到 evidence、检查 calculation support、标记 unsupported claims，并捕捉 approved feedback。
...
这个经验说明：evidence grounding 依赖 **recall、ranking、support validation 和 audit**，不能只看 generation quality。
...
FCA 会自然检查 commentary claims、source evidence 和 underlying calculations 是否能在 analyst review 前 reconcile。

> FCA 会在数字进入 reviewable explanation 之前，检查 commentary 背后的数字是否说得通。
...
在实践中，FCA 会通过 intermediate artifacts 和 final commentary output 一起评估系统质量。系统可以检查 claim extraction、query expansion、candidate recall、evidence linking、support assessment、calculation 和 audit 每一步是否正确运行。
...
这套 harness 让 FCA 更容易 debug，也更安全地演进。它关注每个 claim 在哪里成功、哪里失败，以及为什么，而不是只看 final commentary 是否流畅。
...
FCA 被设计为生成 reviewable artifacts，而不是只有 final text。

### 7. 先调查 failure modes，再优化 final text quality
